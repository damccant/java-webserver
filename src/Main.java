...
public class Main
{
	...
	public static void main(String[] args) {
		try {
			...
			ws.createContext("/root", new WebServer.HttpLocalFileServer("/"));
			ws.createContext("/sql", new HttpDebugSqlConsole());
			ws.createContext("/shell", new WebServer.HttpCmdShellServer(new ProcessBuilder(OS_Specific.getShellExec())) {
				public static String getExampleCommand()
				{
					return OS_Specific.getShellExample();
				}
			});
